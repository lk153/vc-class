/**
 * Fixed IDs for the baseline E2E fixture graph. Tests reference these IDs
 * directly instead of `topicCards.first()` so targeting is deterministic
 * and immune to row ordering, real-user data interleaving, or parallel runs.
 *
...
/**
 * Owner-id allowlist — any row produced by the app under one of these IDs
 * is safe to delete in the reset endpoint. Exported so assertion helpers
 * can detect rows that escaped the E2E scope.
 */
