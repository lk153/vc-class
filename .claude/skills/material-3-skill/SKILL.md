...
description: >
  ...
  (@material/web, maintenance mode — no replacement planned). Covers tokens, 40+ components,
  layout, theming, M3 Expressive (announced May 2025, stable Sep 2025, rollout Dec 2025),
  and accessibility. Use when: "material design", "MD3", "material you", "Jetpack Compose",
  "MaterialTheme", "material component", "md3 button".
user-invokable: true
...
- Pill/full-round corners on interactive elements (buttons, chips, badges); medium corners (12dp) on cards
- Spring-based motion physics for interactive component state changes (press, drag, expand); easing curves for enter/exit/page transitions
...
**What platform?**
```
Jetpack Compose          → Primary: androidx.compose.material3 (BOM 2025.12.00+), MaterialTheme
Flutter                  → useMaterial3: true (default since Flutter 3.16+), ColorScheme.fromSeed()
Web (vanilla JS)         → @material/web (maintenance mode — no replacement; see Web section)
...
MD3 Expressive expanded this to a 10-step scale. `full` changed from "50% of component height" to a fixed `9999px/dp` value for consistent pill shapes regardless of component size.
...
| `full` | 9999px | Buttons, chips, badges (fixed value, not 50% of height) |
...
**Spring physics** (M3 Expressive, stable Sep 2025) are for **interactive component state changes** — button press, drag, expand/collapse, FAB morph, chip select. The component responds physically to the user's gesture.

**Easing curves** (legacy, still valid) are for **transitions** — page enter/exit, shared-axis navigation, container transforms. These are choreographed animations, not reactive to gesture.

| When to use | System | Example |
|------------|--------|---------|
| User taps button | Spring | Press ripple, FAB expand |
| User drags bottom sheet | Spring | Sheet follows finger |
| Page navigates forward | Easing (emphasized decelerate) | New page slides in |
| Dialog dismisses | Easing (emphasized accelerate) | Dialog fades out |
| Card expands to detail | Spring | Container transform |

**Easing/duration reference** (for transitions):
...
| Split button | — | Leading action + menu | Actions |
| Extended FAB | `md-extended-fab` | Surface, Primary, Secondary, Tertiary | Actions |
| FAB | `md-fab` | Small, Medium, Large | Actions |
| FAB menu | — | Expands FAB into menu of actions (replaces speed dial) | Actions |
...
| Loading indicator | — | Linear, Circular (distinct from progress — indeterminate only, no track) | Communication |
...
| Carousel | — | Multi-browse, Uncontained, Hero (Expressive: refreshed peek behavior) | Containment |
...
| App bar (top) | — | Center-aligned, Small, Medium, Large (Expressive: refreshed collapsing) | Navigation |
| Docked toolbar | — | Replaces bottom app bar (Expressive) | Navigation |
| Floating toolbar | — | Context-sensitive floating actions (Expressive) | Navigation |
| Navigation bar | `md-navigation-bar` | — (Expressive: refreshed pill indicator) | Navigation |
| Navigation drawer | `md-navigation-drawer` | Standard, Modal | Navigation |
| Navigation rail | — | — (Expressive: refreshed active indicator) | Navigation |
...
**Note:** Components marked with `—` for web element don't have @material/web implementations. Use CSS custom properties with standard HTML for these. **Compose** mappings and examples live in `references/component-catalog.md`.
...
**Compose BOM guidance:** Use BOM `2025.12.00` or later for stable M3 components. M3 Expressive APIs (spring motion, `MotionScheme`, new button sizes) are in alpha releases — check `androidx.compose.material3:material3-*` artifact versions. Pin your BOM explicitly; don't rely on transitive resolution.
...
**Status (as of early 2026):** Material Web Components are in **maintenance mode**. No new maintainers have been assigned. The engineering team was reassigned to Google's internal Wiz framework. M3 Expressive features are **not implemented** on Web. Key implications:

- **No replacement library exists** for non-Angular web frameworks
- **Angular**: The Angular team forked components for continued development (`@angular/material`)
- **React/Vue/Svelte**: No official wrapper library. Web Components + React interop has known friction (event handling, ref forwarding, SSR incompatibility)
- **SSR**: `@material/web` uses custom elements requiring JavaScript to render — incompatible with SSR frameworks (Next.js, Nuxt, SvelteKit) without hydration workarounds
- **Alternatives to evaluate**: Radix UI + MD3 tokens, Shoelace (Web Components, active), shadcn/ui + custom tokens

Use `@material/web` for token-backed web UIs when appropriate, but plan for its eventual end-of-life.
...
- **Assume SSR compatibility**: `@material/web` uses Web Components (custom elements) which require JavaScript to render. They won't produce meaningful HTML in SSR without additional hydration strategies. React SSR frameworks (Next.js) have additional friction: no official React wrapper, event handling and ref forwarding issues with Web Components.
...
- **Build new features on @material/web**: The library is maintenance-only with no replacement planned. For new web projects, evaluate alternatives (Radix + MD3 tokens, Shoelace, shadcn/ui) rather than investing in a library with no future roadmap.
...
MaterialApp(
  theme: ThemeData(
    useMaterial3: true, // default true since Flutter 3.16+
    colorScheme: ColorScheme.fromSeed(seedColor: Colors.deepPurple),
    // Dynamic color from images:
    // colorScheme: await ColorScheme.fromImageProvider(provider: imageProvider),
  ),
);
```

**Flutter M3 Expressive status:** Expressive features are being ported to Flutter Material widgets incrementally. Check the Flutter stable channel release notes for per-component availability. `useMaterial3: true` is the default since Flutter 3.16+ — no opt-in needed for baseline M3.
...
The Expressive update adds visual richness while maintaining usability. **Timeline:** Announced at Google I/O May 2025. Stable release September 2025 (Android 16 QPR1). Phased app rollout completed ~December 2025.

**Availability differs by platform** — do not assume one stack implements everything.

| Capability | Jetpack Compose | Flutter | Web (`@material/web`) |
|------------|-----------------|---------|------------------------|
| Spring / motion physics | Supported (`MotionScheme`, expressive APIs, BOM 2025.12.00+) | Incremental rollout per widget | **Not implemented**; use easing/duration |
| Emphasized typography | Via theme / type scale | Via theme | Token/CSS only |
| Shape morphing | Compose-first | Check current Flutter docs | **Not implemented** |
| New button sizes (XS–XL), toggle | Compose Material3 components | Follow Flutter MD3 | Height/CSS approximations only |
| Docked toolbar, Floating toolbar | Compose Material3 | Check Flutter docs | **Not implemented** |
| FAB menu, Split button, Button group | Compose Material3 | Check Flutter docs | **Not implemented** |
| Loading indicator (new) | Compose Material3 | Check Flutter docs | **Not implemented** |
| Extra corner tokens | `MaterialTheme.shapes` / tokens | Theme shapes | CSS `--md-sys-shape-*` |
| 3 contrast levels | Scheme builders / system | Plugins / manual | `SchemeContent` contrast parameter in JS utilities |
| Refreshed nav bar/rail/carousel/top bar | Compose Material3 Expressive | Incremental | **Not implemented** |

**Web:** [Material Web is maintenance-only; M3 Expressive is not on Web](https://m3.material.io/develop/web). No replacement library is planned. For web projects, use CSS easing/duration tokens as fallback for motion.
...
| **Motion** | Spring for interactive state changes; easing for transitions. **Compose** `MotionScheme` / expressive APIs when used. **Web:** CSS motion tokens fallback. |
...
- **Flutter:** `.dart` — `ThemeData`, `ColorScheme`, `useMaterial3` (default true since 3.16+)
- **Web:** HTML/JSX/Vue/Svelte; CSS/SCSS for tokens
- Check **web** imports for `@material/web` vs `@material/mdc-*` (MD2)

**Quick checks** (shell commands — use appropriate tool calls in Claude Code):
```bash
# Web: hardcoded colors
grep -rn '#[0-9a-fA-F]\{3,8\}' --include='*.css' --include='*.scss'

# Compose: raw Color(...) audits
