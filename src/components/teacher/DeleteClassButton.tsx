...
export default function DeleteClassButton({
  ...
  const trigger =
    variant === "icon" ? (
      <button
        type="button"
        aria-label={t("classDeleteConfirmTitle")}
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
          setOpen(true);
      ...
      // M3 Danger Ghost (per design/DESIGN.md). Sized to match the status pill beside it.
      <button
        type="button"
        onClick={() => setOpen(true)}
        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-body font-bold text-[#7b0020] bg-[#ffdada]/20 hover:bg-[#ffdada]/40 transition-colors"
      >
      ...
      <ModalOverlay open={open} onClose={() => !deleting && setOpen(false)} panelClass="max-w-md">
        <div className="p-6 md:p-8 text-center">
          ...
          <h3 className="font-body font-bold text-xl text-[#121c2a] mb-2">
            {t("classDeleteConfirmTitle")}
          </h3>
          <p className="text-sm font-body text-[#464554] mb-2 leading-relaxed">
            {t("classDeleteConfirmMessage")}
            <span className="block mt-1 font-bold text-[#121c2a]">&ldquo;{className}&rdquo;</span>
          </p>
          <p className="text-xs font-body text-[#7b0020] mb-6">{t("classDeleteWarning")}</p>
